...
//
// This code is very close to MolThermo and should be considered as a starting point for learning the
// implementation of molecular dynamics in DSEA.
//
...
__global__ void ref_init_block(double * __restrict__ p_in) {
...
__global__ void ref_md_v3a(int32_t i_worker, int32_t order_in, int32_t order_out, double * p_sum,
						...
						double * __restrict__ p_part_U, double * __restrict__ p_part_V, double * __restrict__ p_part_vel,
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		double fx=p_in_c[block_offset_mol+i_mol*block_doubles_per_mol+6];
		double fy=p_in_c[block_offset_mol+i_mol*block_doubles_per_mol+7];
		double fz=p_in_c[block_offset_mol+i_mol*block_doubles_per_mol+8];

		// p_in_c_i32[(block_offset_mol+i_mol*block_doubles_per_mol+9)*2+1]=1;
		...
		for (int cy=-1;cy<2;cy++) {
			...
			if (do_test==true) {
				...
				if (cx==0) {
					...
					n_c_test=p_in_l_i32[block_offset_nm_cell*2+ic_test];		// number of molecules in cell to test
					// n_c_test=p_in_l_i32[block_i_general_nmol*2+0];
				...
				for (int k=0;k<n_c_test;k++) {		// loop over molecules in test cell
					...
					if (do_comp) {
						...
						// if (i_part==50) atomicAdd((int*)&p_debug[0],1);

						if (RIJ2 <= HILF_3) {
							// if (i_part==50) atomicAdd((int*)&p_debug[1],1);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		p_in_c[block_offset_mol+i_mol*block_doubles_per_mol+6]=fx;
...
__global__ void ref_md_v3b(int32_t i_worker, int32_t order_in, int32_t order_out, double * p_sum,
						/*order 0*/ double * __restrict__ p_in_c,
						/*order 1*/ double * __restrict__ p_in_l, double * __restrict__ p_in_r,
						/*order 0*/ double * __restrict__ p_out_c,
						/*order 1*/ double * __restrict__ p_out_l, double * __restrict__ p_out_r,
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		int64_t i_species=p_in_c_i64[block_offset_mol+i_mol*block_doubles_per_mol+12];
...
// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		double px_new=px+vx*md_dt+fx*HILF_1;
		...
		double py_new=py+vy*md_dt+fy*HILF_1;
		double pz_new=pz+vz*md_dt+fz*HILF_1;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (icx==i_part) {
			...
			p_out_c[block_offset_mol+i_store*block_doubles_per_mol+6]=fx;
			p_out_c[block_offset_mol+i_store*block_doubles_per_mol+7]=fy;
			p_out_c[block_offset_mol+i_store*block_doubles_per_mol+8]=fz;
			p_out_c_i64[block_offset_mol+i_store*block_doubles_per_mol+12]=i_species;

			// p_out_c_i32[(block_offset_mol+i_store*block_doubles_per_mol+9)*2]=i_c_part;
			// p_out_c_i32[(block_offset_mol+i_store*block_doubles_per_mol+9)*2+1]=tr;
		}
			...
			p_out_r[block_offset_mol+i_store*block_doubles_per_mol+6]=fx;
			p_out_r[block_offset_mol+i_store*block_doubles_per_mol+7]=fy;
			p_out_r[block_offset_mol+i_store*block_doubles_per_mol+8]=fz;
			p_out_r_i64[block_offset_mol+i_store*block_doubles_per_mol+12]=i_species;

			// p_out_r_i32[(block_offset_mol+i_store*block_doubles_per_mol+9)*2]=i_c_part;
			// p_out_r_i32[(block_offset_mol+i_store*block_doubles_per_mol+9)*2+1]=1;
		}
		else if (icx==i_part-1) {
			// index of molecule in cell list
			...
			p_out_l[block_offset_mol+i_store*block_doubles_per_mol+6]=fx;
			p_out_l[block_offset_mol+i_store*block_doubles_per_mol+7]=fy;
			p_out_l[block_offset_mol+i_store*block_doubles_per_mol+8]=fz;
			p_out_l_i64[block_offset_mol+i_store*block_doubles_per_mol+12]=i_species;

			// p_out_l_i32[(block_offset_mol+i_store*block_doubles_per_mol+9)*2]=i_c_part;
			// p_out_l_i32[(block_offset_mol+i_store*block_doubles_per_mol+9)*2+1]=1;
		}
		else {
...
__global__ void ref_md_thermo_a(double * __restrict__ p_in, double * __restrict__ p_sum) {
...
__global__ void ref_md_thermo_b(double * __restrict__ p_in, double * __restrict__ p_sum) {
	int32_t * p_in_i32 = (int32_t *)p_in;
	int32_t global_id = blockIdx.x*blockDim.x+threadIdx.x;
	int32_t n_mol=p_in_i32[block_i_general_nmol*2+0];
...
__global__ void ref_stat_collect(	double * __restrict__ p_sum_u, double * __restrict__ p_sum_v, double * __restrict__ p_sum_vel,
								double * __restrict__ p_result, int32_t i_part) {
...
__global__ void ref_md_fill_mol_work_list(int32_t * __restrict__ mol_list, int32_t * __restrict__ nm, int32_t * __restrict__ prefix_sum, int32_t n) {
	int32_t global_id = blockIdx.x*blockDim.x+threadIdx.x;
...
void DS::ref_caller_worker (double ** p_in, double ** p_out, int32_t i_part, int32_t i_super_cycle,
...
// MatAdd<<<numBlocks, threadsPerBlock>>>(
	...
	// init output arrays
	if (i_part==0) {
		for (int32_t i=0;i<order_out+1;i++) ref_init_block<<<block_ncc/threads_per_block+1,threads_per_block,0,*stream>>>(p_out[i*2]);
	}
	else if (i_part>=(n_part-order_out)) {
		// no init required

		if (p_out[order_out*2]!=(double*)-1) ref_init_block<<<block_ncc/threads_per_block+1,threads_per_block,0,*stream>>>(p_out[order_out*2]);
	}
	else {
		ref_init_block<<<block_ncc/threads_per_block+1,threads_per_block,0,*stream>>>(p_out[order_out*2]);
	}
	...
	// fill mol work list
	ref_md_fill_mol_work_list <<<block_ncc/threads_per_block+1,threads_per_block,0,*stream>>>((int32_t*)d_mol_list,&my_d_in[block_offset_nm_cell*2],(int32_t*)d_prefix_sum,block_ncc);
	...
	ref_md_v3a <<<blockSize,threads_per_block,0,*stream>>>(iworker,order_in,order_out,(double*)d_sum,
												p_in[0],p_in[1],p_in[2],
												p_out[0],p_out[1],p_out[2],
												(int32_t*)d_mol_list,
												(int32_t*)d_debug,
												(double*)d_part_u,(double*)d_part_v,(double*)d_part_vel,
												(int64_t*)d_sum_N);
	...
	ref_md_thermo_a <<<128,threads_per_block,0,*stream>>>(p_in[0],(double*)d_sum);
	ref_md_thermo_b <<<1,threads_per_block,0,*stream>>>(p_in[0],(double*)d_sum);

	ref_stat_collect <<<1,1,0,*stream>>> (	(double*)d_sum_u,(double*)d_sum_v,(double*)d_sum_vel,
										(double*)d_result,i_part);

	ref_md_v3b <<<blockSize,threads_per_block,0,*stream>>>(iworker,order_in,order_out,(double*)d_sum,
												p_in[0],p_in[1],p_in[2],
												p_out[0],p_out[1],p_out[2],
												(int32_t*)d_mol_list,
												(int32_t*)d_debug);
